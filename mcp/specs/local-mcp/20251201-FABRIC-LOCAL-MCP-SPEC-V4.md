...
- **Not JetBrains (Yet):** IDE support limited to VS Code for M1/M2. JetBrains deferred post-GA.
...
│  • publicapis list/get            │  • upload_file                      │
│  • itemdefinition get             │  • download_file                    │
│  • search_documentation           │  • create_item / update_item        │
│  • get_fabric_glossary            │  • delete_item                      │
│  • generate_cli_script            │  • create_deployment (dry_run)      │
└───────────┬─────────────────────────────────────────┬───────────────────┘
            │                                         │
            ▼                                         ▼
    ┌───────────────┐                         ┌───────────────────────────┐
    │  Local Files  │                         │      FABRIC CLOUD         │
    │  Git / Schemas│                         │  (REST APIs + OneLake)    │
    └───────────────┘                         └───────────────────────────┘
```
...
| Concept | Description |
|---------|-------------|
| **Context Tools** | Read-only, run locally, no auth required. Provide schemas, docs, examples. |
...
|-----------|--------|-------|------------------|
| **Current** | Live | Context & Validation | OpenAPI specs, JSON schemas, documentation search |
| **M1** | Q1 2026 | VS Code Extension | One-click setup, integrated auth, status UI |
| **M2** | Q2 2026 | Execution Tools | Upload/download, item CRUD, CLI generation, dry run |
...
### 7.2 M1 (Q1 2026) — VS Code Extension

| Pri | Category | I can... | Requester | Status |
|-----|----------|----------|-----------|--------|
| P0 | Setup | Install and configure in under 2 minutes | Developer | 🔄 |
| P0 | Auth | Authenticate once and stay logged in (token cached in OS keychain) | Developer | 🔄 |
...
| P1 | Errors | See specific field and expected format in validation errors | Developer | ✅ |

### 7.3 M2 (Q2 2026) — Execution Tools

| Pri | Category | I can... | Requester | Status |
|-----|----------|----------|-----------|--------|
| P0 | Data | Upload local files to a Lakehouse (up to 500MB streaming) | Data Engineer | 🔄 |
| P0 | Data | Download data from Fabric to work on locally | Data Engineer | 🔄 |
| P0 | Items | Deploy a new item (Lakehouse, Warehouse) from my IDE | Developer | 🔄 |
| P0 | Items | Undo a deployment if I make a mistake (delete immediately) | Developer | 🔄 |
...
|---|----------|----------------|
| Q1 | Should we support JetBrains IDEs in M1? | Defer to post-GA. |
| Q2 | How do we handle large file uploads (>1GB)? | Chunked upload via OneLake DFS API. |
| Q3 | Should we use MCP Resources (static) vs Tools (dynamic)? | Yes—Resources for glossary, templates. Tools for execution. |
| Q4 | What's the distinction between "Blueprint" and "Template"? | Requires PM alignment with Unified Copilot team. |
| Q5 | How do we coordinate with OneLake MCP team on file operations? | Cross-team meeting needed. May scope Local MCP to item-level only. |
| Q6 | Should `generate_cli_script` require Fabric CLI installed? | Engineering investigating hybrid approach. |
...
| VS Code extension approval delays | Medium | Early submission, follow marketplace guidelines |

---

## 10. Resources & References

- **GitHub Repository:** [github.com/microsoft/fabric-local-mcp](https://github.com/microsoft/fabric-local-mcp) *(open-source)*
...
- **VS Code Extension Guidelines:** [VS Code Marketplace Publishing](https://code.visualstudio.com/api/working-with-extensions/publishing-extension)
...
| Tool | Operation | Description | Status |
|------|-----------|-------------|--------|
| `download_file` | Read | Download file from OneLake to local filesystem | 🔄 |
| `upload_file` | Write | Upload local file to OneLake (up to 500MB) | 🔄 |
...
|------|------------|--------|
| OneLake MCP | Coordinate file operation scope (avoid overlap) | 🔄 Meeting needed |
